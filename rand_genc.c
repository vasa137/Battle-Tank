...
void rand_gen()
{
	...
	{	
		...
		if ((matrix[h - 2][w - 2] == ' ') && (matrix[h - 2][w - 2] != 'g')) // if he found a blank spot in the matrix, he will print a random character
		{
			...
			{
				if (matrix[h - 3][l - 2] == ' ') what_to_print(h - 1, l, h - 1, l, possibility[pos]), matrix[h - 3][l - 2] = possibility[pos];
				if (matrix[h - 2][l - 2] == ' ') what_to_print(h, l, h, l, possibility[pos]), matrix[h - 2][l - 2] = possibility[pos];
				if (matrix[h - 1][l - 2] == ' ') what_to_print(h + 1, l, h+1, l, possibility[pos]), matrix[h - 1][l - 2] = possibility[pos];
